...
レプリケーションとは、**データベースが持つデータを他のサーバに複製し、データの可用性と信頼性を高める手法**です。
...
このように、即座の整合性は保証しないが、最終的には全てのレプリカが最新のデータに追いつくことを保存する整合性レベルを、「**結果整合性**」と呼びます。
- メリット：書き込み処理が高速
...
sequenceDiagram
  ...
  Master->>Replica1: データ複製
  rect rgba(0, 255, 0, .1)
  Note over Replica1: 非同期で処理する。レプリカ2からのレスポンスは待たない
    Master->>Replica2: データ複製
  end
  Replica1-->>Master: 書き込み確認
  Master-->>Client: 書き込み完了
...
```
### モノトニックな読み取り
非同期レプリケーションが生じる問題の2つ目の例は、**過去に遡って**データを読み取ってしまう可能性です。
...
| 整合性レベル | 使用ケース | メリット | デメリット |
| --- | --- | --- | --- |
| 強い整合性 | 金融取引、決済 | データ整合性を即座に保証 | 書き込み遅延が発生 |
| 結果整合性 | SNSのタイムライン | 応答が高速で、スケーラブル | 一時的な不整合の可能性 |
